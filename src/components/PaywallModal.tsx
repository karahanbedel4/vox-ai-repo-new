      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl">
        ...
        >
          ...
          <div className="p-5 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 text-xs">
            ...
            {/* Plan Selection Cards: Monthly, Yearly, Lifetime */}
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  ...
                  className={`relative p-3 rounded-2xl border text-left transition-all flex flex-col justify-between ${
                    selectedPlan === 'monthly'
                      ? 'bg-primary/20 border-primary text-white shadow-sm ring-1 ring-primary'
                ...
                >
                  <div>
                    ...
                    <div className="font-display text-sm font-bold text-white mt-1">49.99 ₺</div>
                  </div>
                  <span className="text-[9px] text-on-surface-variant mt-2 block">İstediğin an iptal</span>
                </button>

                {/* Yearly Plan (Best Value) */}
                <button
                  id="paywall-plan-yearly"
                  onClick={() => setSelectedPlan('yearly')}
                  className={`relative p-3 rounded-2xl border text-left transition-all flex flex-col justify-between ${
                    selectedPlan === 'yearly'
                      ? 'bg-primary/25 border-primary text-white shadow-sm ring-1 ring-primary'
                      : 'bg-surface-container-high/40 border-white/10 text-on-surface-variant hover:border-white/20'
                  }`}
                >
                  <div className="absolute -top-2 right-1 bg-gradient-to-r from-emerald-400 to-primary text-on-primary text-[8px] font-extrabold px-1.5 py-0.5 rounded-full uppercase tracking-wider shadow-sm">
                    POPÜLER
                  </div>
                  <div>
                    <span className="text-[9px] uppercase font-bold text-primary tracking-wider block">Yıllık</span>
                    <div className="font-display text-sm font-bold text-white mt-1">399.99 ₺</div>
                    <span className="text-[9px] text-emerald-400 font-semibold block">33.30 ₺/ay</span>
                  </div>
                ...
                >
                  <div className="absolute -top-2 right-1 bg-amber-400 text-neutral-950 text-[8px] font-black px-1.5 py-0.5 rounded-full uppercase tracking-wider shadow-sm flex items-center gap-0.5">
                  ...
                  <span className="text-[9px] text-amber-300 font-bold mt-2 block">Tek seferlik</span>
                </button>
              </div>
          ...
          <div className="p-5 border-t border-white/10 bg-surface-container space-y-3">
            <button
              ...
              onClick={handleBuy}
              disabled={isLoading}
              className="w-full py-4 bg-primary text-on-primary font-bold text-sm rounded-2xl shadow-md active:scale-95 hover:opacity-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {isLoading ? (
                <>
                  <RefreshCw className="w-4 h-4 animate-spin" />
                  <span>RevenueCat İşleniyor...</span>
                </>
              ) : (
                <>
                  ...
                  <span>
                    {selectedPlan === 'lifetime'
                      ? 'Ömür Boyu Pro Satın Al (999.99 ₺)'
                      : selectedPlan === 'yearly'
                      ? '7 Gün Ücretsiz Dene & Yıllık Başlat'
                      : 'Aylık Vox Pro Başlat (49.99 ₺/ay)'}
                  </span>
                </>
              )}
            </button>
            ...
            <div className="flex items-center justify-center gap-1 text-[10px] text-emerald-400/80">
              ...
              <span>RevenueCat & Apple StoreKit Güvenli Ödeme</span>
            </div>
          </div>
        </motion.div>

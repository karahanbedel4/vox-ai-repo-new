...
export const ResumeBanner: React.FC<ResumeBannerProps> = ({
  ...
  return (
    <div className="relative overflow-hidden bg-surface-container-high/95 backdrop-blur-xl border border-white/10 p-3.5 rounded-2xl shadow-md animate-fade-in group hover:border-primary/40 transition-all">
      {/* Background Accent */}
      <div className="absolute top-0 left-0 w-1/3 h-full bg-primary/5 pointer-events-none" />
      ...
      <div className="flex items-center justify-between gap-3 relative z-10 pt-1">
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1 cursor-pointer" onClick={onResume}>
          <div className="relative w-11 h-11 rounded-xl overflow-hidden shrink-0 bg-surface-container border border-white/10 shadow-sm flex items-center justify-center">
            {article.imageUrl ? (
              <img
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            onClick={onResume}
            className="bg-primary text-on-primary px-3.5 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 active:scale-95 transition-transform shadow-sm"
          >

...
export class WoodRainSynthEngine {
  ...
  constructor() {
    // Keep audio synthesis active in background for continuous focus audio
  }
  ...
  // Play a soothing crystal meditation bell / chime for Pomodoro timer completion
  public playBellChime() {
    try {
      this.initCtx();
      if (!this.ctx) return;

      const baseFreqs = [528, 1056, 1584];
